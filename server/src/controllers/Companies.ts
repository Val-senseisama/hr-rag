...
export const createCompany = asyncHandler(async (req: any, res: any) => {
  ...
  const adminRole = {
    company: company._id as unknown as mongoose.Types.ObjectId,
    read: 1,
    create: 1,
    update: 1,
    delete: 1
  ...
  if (user) {
    user.companies.push(company._id as unknown as mongoose.Types.ObjectId);
    
    // Remove any existing role for this company and add admin role
    user.role = user.role.filter((r: any) => String(r.company) !== String(company._id));
...
export const updateCompany = asyncHandler(async (req: any, res: any) => {
  ...
  const userRole = user.role.find((r: any) => String(r.company) === String(companyId));
...
export const addUserToCompany = asyncHandler(async (req: any, res: any) => {
  const { companyId, userId } = req.params;
  const authUser = (req as any).user;
  if (!authUser) return res.status(401).json({ message: "Unauthorized" });
  if (!companyId || !userId) return res.status(400).json({ message: "companyId and userId are required" });

  const company = await Company.findById(companyId);
  const user = await User.findById(userId);
  if (!company || !user) return res.status(404).json({ message: "Company or User not found" });
  ...
  const userRole = requestingUser.role.find((r: any) => String(r.company) === String(companyId));
  ...
  const companyObjectId = company._id as unknown as mongoose.Types.ObjectId;
  ...
  if (!user.companies.some((c) => String(c) === String(companyObjectId))) {
    ...
    user.role = user.role.filter((r: any) => String(r.company) !== String(companyId));
...
export const removeUserFromCompany = asyncHandler(async (req: any, res: any) => {
  const { companyId, userId } = req.params;
  if (!companyId || !userId) return res.status(400).json({ message: "companyId and userId are required" });
...
export const getUserCompanies = asyncHandler(async (req: any, res: any) => {
...
export const deleteCompany = asyncHandler(async (req: any, res: any) => {
  const { companyId } = req.params;
  const company = await Company.findById(companyId);
  if (!company) return res.status(404).json({ message: "Company not found" });
...
export const inviteUserByEmail = asyncHandler(async (req: any, res: any) => {
  const { companyId } = req.params;
  ...
  const inviterRole = inviter.role.find((r: any) => String(r.company) === String(companyId));
  ...
  if (existingUser) {
    ...
    if (!isInCompany) {
      company.users.push(existingUser._id as unknown as mongoose.Types.ObjectId);
      await company.save();
    }

    const hasCompany = existingUser.companies.some((c) => String(c) === String(company._id));
    if (!hasCompany) {
      existingUser.companies.push(company._id as unknown as mongoose.Types.ObjectId);
      // Default role: Viewer
      const defaultRole = {
        company: company._id as unknown as mongoose.Types.ObjectId,
        read: 1,
...
export const joinCompanyByToken = asyncHandler(async (req: any, res: any) => {
  ...
  if (!company) return res.status(404).json({ message: "Invalid or expired token" });
  ...
  // Add user to company
  company.users.push(authUser.id as unknown as mongoose.Types.ObjectId);
...
export const removeMemberFromCompany = asyncHandler(async (req: any, res: any) => {
  const { companyId, userId } = req.params;
  const authUser = (req as any).user;
  ...
  const targetUserRole = targetUser.role.find((r: any) => String(r.company) === String(companyId));
  ...
  targetUser.role = targetUser.role.filter((r: any) => String(r.company) !== String(companyId));
...
export const updateMemberPermissions = asyncHandler(async (req: any, res: any) => {
  const { companyId, userId } = req.params;
  ...
  if (deletePerm === 0) {
    ...
    if (owners.length <= 1 && targetUser.role.some((r: any) => String(r.company) === String(companyId) && r.delete === 1)) {
  ...
  const newRole = {
    company: companyId as unknown as mongoose.Types.ObjectId,
...
export const getCompanyMembers = asyncHandler(async (req: any, res: any) => {
  const { companyId } = req.params;
  const authUser = (req as any).user;
  ...
  const members = company.users.map((user: any) => ({
    _id: user._id,
    name: user.name,
    email: user.email,
    role: user.role.find((r: any) => String(r.company) === String(companyId)) || {
      read: 0,
      create: 0,
      update: 0,
      delete: 0
