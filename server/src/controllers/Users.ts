...
export const register = asyncHandler(async (req: any, res: any) => {
  ...
  const access = await setJwt(user._id as unknown as mongoose.Types.ObjectId);
  const refresh = await setRefreshJwt(user._id as unknown as mongoose.Types.ObjectId);
