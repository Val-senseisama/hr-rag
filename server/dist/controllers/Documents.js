...
export function hasCompanyPermission(user, companyId, action) {
...
export const createDocument = asyncHandler(async (req, res) => {
    ...
    const looksLikePdf = ((typeof contentType === 'string' && contentType.toLowerCase().includes('pdf')) ||
        (typeof filename === 'string' && filename.toLowerCase().endsWith('.pdf')));
    const looksLikeDocx = ((typeof contentType === 'string' && (contentType.toLowerCase().includes('application/vnd.openxmlformats-officedocument.wordprocessingml.document') ||
    ...
    try {
        const textToEmbed = (typeof content === "string" && content.trim().length > 0) ? content.trim() :
        ...
        if (textToEmbed) {
            ...
            const makeChunks = (text, maxLen = 2000, overlap = 200) => {
                const chunks = [];
...
export const listCompanyDocuments = asyncHandler(async (req, res) => {
...
export const downloadDocumentFile = asyncHandler(async (req, res) => {
    ...
    const stream = openDownloadStream(existing.fileId);
